...
import (
	...
	"harness-onboarder/internal/state"
)
...
func init() {
	...
	rootCmd.Flags().String("state-file", ".harness-onboarder-state.json", "State file for incremental runs")
...
func setDefaults() {
	...
	if viper.IsSet("state-file") {
		config.Runtime.StateFile = viper.GetString("state-file")
	}
	...
	if config.Runtime.StateFile == "" {
		config.Runtime.StateFile = ".harness-onboarder-state.json"
	}
...
func runOnboarder(cmd *cobra.Command, args []string) error {
	...
	stateManager, err = state.NewManager(config.Runtime.StateFile)
	if err != nil {
...
func filterRepositories(repos []models.Repository) []models.Repository {
	...
	for _, repo := range repos {
		...
		// Always include repo - let individual processing decide what to do
		// State is kept for logging/reporting only, not for filtering
...
func processYAMLMode(ctx context.Context, repos []models.Repository) error {
	...
	for _, repo := range repos {
		go func(r models.Repository) {
			...
			results <- processRepositoryYAML(ctx, r)
		}(repo)
	}
...
func processAPIMode(ctx context.Context, repos []models.Repository) error {
	...
	for _, repo := range repos {
		go func(r models.Repository) {
			...
			time.Sleep(config.Runtime.RateLimit)
			results <- processRepositoryAPI(ctx, r)
		}(repo)
	}
...
func processRepositoryYAML(ctx context.Context, repo models.Repository) error {
	...
	if err != nil {
		stateManager.RecordError(repo.FullName, err)
		return fmt.Errorf("failed to create PR for %s: %w", repo.FullName, err)
	}
	
	stateManager.RecordSuccess(repo.FullName, repo.PushedAt, "yaml")
	return nil
}
...
func processRepositoryAPI(ctx context.Context, repo models.Repository) error {
	...
	if err != nil {
		stateManager.RecordError(repo.FullName, err)
		return fmt.Errorf("failed to create component for %s: %w", repo.FullName, err)
	}
	
	stateManager.RecordSuccess(repo.FullName, repo.PushedAt, "api")
	return nil
}
...
func processRegisterMode(ctx context.Context, repos []models.Repository) error {
	...
	for _, repo := range repos {
		go func(r models.Repository) {
			...
			time.Sleep(config.Runtime.RateLimit)
			results <- processRepositoryRegister(ctx, r)
		}(repo)
	}
...
func processRepositoryRegister(ctx context.Context, repo models.Repository) error {
	...
	if err != nil {
		// Don't treat missing catalog files as errors - just skip silently
	...
	if err != nil {
		stateManager.RecordError(repo.FullName, err)
		return fmt.Errorf("failed to register entity import for %s: %w", repo.FullName, err)
	}
	
	stateManager.RecordSuccess(repo.FullName, repo.PushedAt, "register")
	return nil
}
